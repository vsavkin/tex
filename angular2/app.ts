...
import * as n from "@reactivex/rxjs"; // this should not be needed
console.log(n, n.Observable.prototype);

import {Tabs, TabTitle, TabContent} from './tabs';
import {ConfTalks} from './conf_talks';

@Component({
  selector:'ng-demo',
	directives: [ConfTalks, Tabs, TabContent, TabTitle, CORE_DIRECTIVES, FORM_DIRECTIVES],
	template: `
    ...
    <tabs>
      ...
      <template class="tab-title" [ng-if]="eventsVisible">
			   <tab-title>Events</tab-title>
      </template>
      <template class="tab-content" [ng-if]="eventsVisible">
        <tab-content>
          <ul>
            <li>Slide review</li>
            <li>Speaker diner</li>
